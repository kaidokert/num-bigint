...
impl fmt::Display for BigInt {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.pad_integral((self.sign() != Minus), "", &self.data.to_str_radix(10))
    }
}

impl fmt::Binary for BigInt {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.pad_integral((self.sign() != Minus), "0b", &self.data.to_str_radix(2))
    }
}

impl fmt::Octal for BigInt {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.pad_integral((self.sign() != Minus), "0o", &self.data.to_str_radix(8))
    }
}

impl fmt::LowerHex for BigInt {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.pad_integral((self.sign() != Minus), "0x", &self.data.to_str_radix(16))
    }
}
...
impl fmt::UpperHex for BigInt {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        f.pad_integral((self.sign() != Minus), "0x", &s)
    }
}
...
impl Roots for BigInt {
    ...
    fn sqrt(&self) -> Self {
        assert!((self.sign() != Minus), "square root is imaginary");
...
// const-num-traits: `FromBytes` stays borrowed + `?Sized` (unsized `[u8]`, no
// alloc — unchanged from upstream); `ToBytes` returns owned by value.
