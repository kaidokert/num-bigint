...
// const-num-traits keeps `FromBytes` borrowed + `?Sized` (it reads the buffer,
// doesn't consume it), so the variable-length `type Bytes = [u8]` slice impl is
// unchanged from upstream — no allocation. `ToBytes` is by value (the fork's
// owned-return convention; rescue non-consuming reads via `impl ToBytes for &T`
// if needed).
